...
#define function make_pkt() which takes arguments seqNum, data, checksum
    #packet contains seqNum,data and checksum. So assign required values to variable 'data' below. SeqNum is already assigned  
    packet=str(int(seqNum)).encode()+ #fill in start #fill in end
    #return packet 
#fill in end
    ...
    if rcvpkt:
        if seqNum:
            if (rdt_rcv(rcvpkt) and (corrupt(rcvpkt) or has_seq0(rcvpkt))):
                checksum=make_checksum(str(int(not seqNum)).encode()+b'ACK') #Creating a checksum using seqNum and 'ACK'
		#fill in start
		sndpkt=#make a packet by calling make pkt with arguments seqNum, 'ACK', checksum. (Note that seqNum type is boolean and to send string in python 3 we send with following format: 			b'string', where b signifies bytes and 'string' signifies the string you intend to send)
		#fill in end
                udt_send(sndpkt)
